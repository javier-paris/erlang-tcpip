%%%-------------------------------------------------------------------
%%% File    : established.erl
%%% Author  : Javier Paris Fernandez <javier.paris@udc.es>
%%% Description : Tcp Established connection state
%%%
%%% Created : 19 Aug 2004 by Javier Paris Fernandez <javier.paris@udc.es>
%%%
%%%
...
recv(Tcb, Pkt, Writer) ->
    tcp_input:process_packet(Tcb, Pkt, established, Writer).
...
send(Tcb, {send, fin}) ->
	...
	Data_Size == 0 ->
	    ...
	    tcb:syncset_tcbdata(Tcb, state, fin_wait_1);
	true ->
	    tcp_packet:send_packet(Tcb, data),
...
out_order_action(Tcb, Data, Writer) ->
    ...
    tcp_con:send_packet(Writer, ack). % For fast retransmit
